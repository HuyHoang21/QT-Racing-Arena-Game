    ...
    , m_chatModel(new QStandardItemModel(this)) // create the model to hold the messages
{
    ...
    ui->showQuestion->setDisabled(true);
    ui->showScore->setDisabled(true);
    ui->showScore->setPlainText("0");
    ...
    connect(m_chatClient, &ChatClient::questionArrive, this, &ChatWindow::questionArrive);
    connect(m_chatClient, &ChatClient::resultArrive, this, &ChatWindow::resultArrive);
    ...
    connect(ui->answerButton, &QPushButton::clicked, this, &ChatWindow::sendAnswer);
    connect(ui->answerEdit, &QLineEdit::returnPressed, this, &ChatWindow::sendAnswer);
}
...
void ChatWindow::sendAnswer(){
    if(ui->answerEdit->text().isEmpty())
        return;
    ui->answerButton->setDisabled(true);
    m_chatClient->sendAnswer(ui->answerEdit->text());
    ui->answerEdit->clear();
}
...
void ChatWindow::questionArrive(const QString &question){
    ui->showQuestion->setPlainText(question);
    ui->answerButton->setDisabled(false);
}

void ChatWindow::resultArrive(const bool &result){
    if(result == true) m_score += 1;
    qDebug() << "resultArrive called!";
    ui->showScore->setPlainText(QString::number(m_score));
}
