...
void ChatClient::sendAnswer(const QString &text)
{
    if (text.isEmpty())
        return; // We don't send empty messages
    // create a QDataStream operating on the socket
...
void ChatClient::jsonReceived(const QJsonObject &docObj)
{
    ...
    } else if (typeVal.toString().compare(QLatin1String("questionarrive"), Qt::CaseInsensitive) == 0){
        const QJsonValue questionVal = docObj.value(QLatin1String("question"));
        if (questionVal.isNull() || !questionVal.isString())
            return;
        emit questionArrive(questionVal.toString());
    } else if (typeVal.toString().compare(QLatin1String("result"), Qt::CaseInsensitive) == 0){
        const QJsonValue resultVal = docObj.value(QLatin1String("result"));
        if (resultVal.isNull() || !resultVal.isBool())
            return;
        emit resultArrive(resultVal.toBool());
    }
}
